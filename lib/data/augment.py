...
import scipy.signal
import torch
from lib.components import persistence, nv_misc
from lib.components.ops import upfirdn2d, conv2d_gradfix, grid_sample_gradfix
...
augpipe_specs = {
    ...
    'human_bgc': dict(xflip=1, rotate90=1, rotate_max=0.05, xint=1, scale=1, rotate=1, aniso=1, xfrac=1, brightness=1, contrast=1, lumaflip=1, hue=1, saturation=1),
}
...
class AugmentPipe(torch.nn.Module):
    def __init__(self,
        xflip=0, rotate90=0, rotate180=0, xint=0, xint_max=0.125,
    ...
    def forward(self, images, debug_percentile=None):
        ...
        # Apply 180 degree rotations with probability (rotate90 * strength).
        if self.rotate180 > 0:
            i = torch.floor(torch.rand([batch_size], device=device) * 4)
            i = torch.where(torch.rand([batch_size], device=device) < self.rotate180 * self.p, i, torch.zeros_like(i))
            if debug_percentile is not None:
                i = torch.full_like(i, torch.floor(debug_percentile * 4))
            G_inv = G_inv @ rotate2d_inv(-np.pi * i)
